...
void
TM_SPI_Init(SPI_TypeDef* SPIx, TM_SPI_PinsPack_t pinspack) {
...
void
TM_SPI_InitWithMode(SPI_TypeDef* SPIx, TM_SPI_PinsPack_t pinspack, TM_SPI_Mode_t SPI_Mode) {
...
uint16_t
TM_SPI_GetPrescalerFromMaxFrequency(SPI_TypeDef* SPIx, uint32_t MAX_SPI_Frequency) {
...
TM_SPI_DataSize_t
TM_SPI_SetDataSize(SPI_TypeDef* SPIx, TM_SPI_DataSize_t DataSize) {
...
void
TM_SPI_ReadMulti(SPI_TypeDef* SPIx, uint8_t* dataIn, uint8_t dummy, uint32_t count) {
    uint32_t i;
...
void
TM_SPI_SendMulti16(SPI_TypeDef* SPIx, uint16_t* dataOut, uint16_t* dataIn, uint32_t count) {
    uint32_t i;
...
void
TM_SPI_WriteMulti16(SPI_TypeDef* SPIx, uint16_t* dataOut, uint32_t count) {
    uint32_t i;
...
void
TM_SPI_ReadMulti16(SPI_TypeDef* SPIx, uint16_t* dataIn, uint16_t dummy, uint32_t count) {
    uint32_t i;
...
__weak void
TM_SPI_InitCustomPinsCallback(SPI_TypeDef* SPIx, uint16_t AlternateFunction) {
...
/* Private functions */
static void
TM_SPIx_Init(SPI_TypeDef* SPIx, TM_SPI_PinsPack_t pinspack, TM_SPI_Mode_t SPI_Mode, uint16_t SPI_BaudRatePrescaler, uint16_t SPI_MasterSlave, uint16_t SPI_FirstBit) {
...
#ifdef USE_SPI1
void
TM_SPI1_INT_InitPins(TM_SPI_PinsPack_t pinspack) {
    /* Init SPI pins */
#if defined(GPIOA)
...
#ifdef USE_SPI2
void
TM_SPI2_INT_InitPins(TM_SPI_PinsPack_t pinspack) {
    /* Init SPI pins */
...
#ifdef USE_SPI3
void
TM_SPI3_INT_InitPins(TM_SPI_PinsPack_t pinspack) {
    /* Enable SPI pins */
#if defined(GPIOB)
    if (pinspack == TM_SPI_PinsPack_1) {
...
#ifdef USE_SPI4
void
TM_SPI4_INT_InitPins(TM_SPI_PinsPack_t pinspack) {
    /* Init SPI pins */
#if defined(GPIOE)
    if (pinspack == TM_SPI_PinsPack_1) {
...
#ifdef USE_SPI5
void
TM_SPI5_INT_InitPins(TM_SPI_PinsPack_t pinspack) {
    /* Init SPI pins */
#if defined(GPIOF)
    if (pinspack == TM_SPI_PinsPack_1) {
...
#ifdef USE_SPI6
void
TM_SPI6_INT_InitPins(TM_SPI_PinsPack_t pinspack) {
#if defined(GPIOG)
    if (pinspack == TM_SPI_PinsPack_1) {
        /* Init SPI pins */
