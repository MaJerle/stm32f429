...
uint32_t
TM_RTC_Init(TM_RTC_ClockSource_t source) {
    uint32_t status;
    uint8_t stat = 1;
    TM_RTC_t datatime;
...
TM_RTC_Result_t
TM_RTC_SetDateTime(TM_RTC_t* data, TM_RTC_Format_t format) {
    TM_RTC_t tmp;
...
TM_RTC_Result_t
TM_RTC_SetDateTimeString(char* str) {
    TM_RTC_t tmp;
    uint8_t i = 0;
...
void
TM_RTC_GetDateTime(TM_RTC_t* data, TM_RTC_Format_t format) {
    uint32_t unix;
...
uint8_t
TM_RTC_GetDaysInMonth(uint8_t month, uint8_t year) {
    /* Check input data */
    if (
        month == 0 ||
        month > 12
...
uint16_t
TM_RTC_GetDaysInYear(uint8_t year) {
    /* Return days in year */
    return RTC_DAYS_IN_YEAR(2000 + year);
}

void
TM_RTC_Config(TM_RTC_ClockSource_t source) {
    if (source == TM_RTC_ClockSource_Internal) {
        /* Enable the LSI OSC */
        RCC_LSICmd(ENABLE);
...
void
TM_RTC_Interrupts(TM_RTC_Int_t int_value) {
    uint32_t int_val;
...
uint32_t
TM_RTC_GetUnixTimeStamp(TM_RTC_t* data) {
...
void
TM_RTC_GetDateTimeFromUnix(TM_RTC_t* data, uint32_t unix) {
    uint16_t year;
...
void
TM_RTC_SetAlarm(TM_RTC_Alarm_t Alarm, TM_RTC_AlarmTime_t* DataTime, TM_RTC_Format_t format) {
...
void
TM_RTC_DisableAlarm(TM_RTC_Alarm_t Alarm) {
    switch (Alarm) {
        case TM_RTC_Alarm_A:
            /* Disable Alarm A */
...
void
TM_RTC_WriteBackupRegister(uint8_t location, uint32_t value) {
...
uint32_t
TM_RTC_ReadBackupRegister(uint8_t location) {
    /* Check input, 0 to 18 registers are allowed */
    if (location > 18) {
        return 0;
    }
