...
void
TM_OneWire_Init(TM_OneWire_t* OneWireStruct, GPIO_TypeDef* GPIOx, uint16_t GPIO_Pin) {
...
uint8_t
TM_OneWire_Reset(TM_OneWire_t* OneWireStruct) {
    uint8_t i;
...
void
TM_OneWire_WriteBit(TM_OneWire_t* OneWireStruct, uint8_t bit) {
    if (bit) {
        /* Set line low */
        ONEWIRE_LOW(OneWireStruct);
        ONEWIRE_OUTPUT(OneWireStruct);
...
uint8_t
TM_OneWire_ReadBit(TM_OneWire_t* OneWireStruct) {
    uint8_t bit = 0;
...
void
TM_OneWire_WriteByte(TM_OneWire_t* OneWireStruct, uint8_t byte) {
    uint8_t i = 8;
    /* Write 8 bits */
    while (i--) {
        /* LSB bit is first */
...
uint8_t
TM_OneWire_ReadByte(TM_OneWire_t* OneWireStruct) {
    uint8_t i = 8, byte = 0;
    while (i--) {
        byte >>= 1;
...
uint8_t
TM_OneWire_First(TM_OneWire_t* OneWireStruct) {
    /* Reset search values */
...
uint8_t
TM_OneWire_Next(TM_OneWire_t* OneWireStruct) {
    /* Leave the search state alone */
    return TM_OneWire_Search(OneWireStruct, ONEWIRE_CMD_SEARCHROM);
}

void
TM_OneWire_ResetSearch(TM_OneWire_t* OneWireStruct) {
...
uint8_t
TM_OneWire_Search(TM_OneWire_t* OneWireStruct, uint8_t command) {
...
int
TM_OneWire_Verify(TM_OneWire_t* OneWireStruct) {
...
void
TM_OneWire_TargetSetup(TM_OneWire_t* OneWireStruct, uint8_t family_code) {
    uint8_t i;
...
void
TM_OneWire_FamilySkipSetup(TM_OneWire_t* OneWireStruct) {
...
uint8_t
TM_OneWire_GetROM(TM_OneWire_t* OneWireStruct, uint8_t index) {
    return OneWireStruct->ROM_NO[index];
}

void
TM_OneWire_Select(TM_OneWire_t* OneWireStruct, uint8_t* addr) {
    uint8_t i;
...
void
TM_OneWire_SelectWithPointer(TM_OneWire_t* OneWireStruct, uint8_t* ROM) {
    uint8_t i;
    TM_OneWire_WriteByte(OneWireStruct, ONEWIRE_CMD_MATCHROM);
...
void
TM_OneWire_GetFullROM(TM_OneWire_t* OneWireStruct, uint8_t* firstIndex) {
    uint8_t i;
    for (i = 0; i < 8; i++) {
...
uint8_t
TM_OneWire_CRC8(uint8_t* addr, uint8_t len) {
