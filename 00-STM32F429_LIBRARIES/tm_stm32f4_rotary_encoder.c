...
void
TM_RE_Init(TM_RE_t* data, GPIO_TypeDef* GPIO_A_Port, uint16_t GPIO_A_Pin, GPIO_TypeDef* GPIO_B_Port, uint16_t GPIO_B_Pin) {
    /* Save parameters */
...
void
TM_RE_SetMode(TM_RE_t* data, TM_RE_Mode_t mode) {
    /* Set mode */
    data->Mode = mode;
}

void
TM_RE_Process(TM_RE_t* data) {
    uint8_t now_a;
    uint8_t now_b;
