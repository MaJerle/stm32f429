...
void
TM_DELAY_EnableDelayTimer(void) {
...
void
TM_DELAY_DisableDelayTimer(void) {
#if defined(TM_DELAY_TIM)
...
TM_DELAY_Timer_t*
TM_DELAY_TimerCreate(uint32_t ReloadValue, uint8_t AutoReload, uint8_t StartTimer, void (*TM_DELAY_CustomTimerCallback)(void*), void* UserParameters) {
...
void
TM_DELAY_TimerDelete(TM_DELAY_Timer_t* Timer) {
    uint8_t i;
    uint32_t irq;
    TM_DELAY_Timer_t* tmp;
...
TM_DELAY_Timer_t*
TM_DELAY_TimerStop(TM_DELAY_Timer_t* Timer) {
    /* Disable timer */
    Timer->Enabled = 0;
...
TM_DELAY_Timer_t*
TM_DELAY_TimerStart(TM_DELAY_Timer_t* Timer) {
    /* Enable timer */
    Timer->Enabled = 1;
...
TM_DELAY_Timer_t*
TM_DELAY_TimerReset(TM_DELAY_Timer_t* Timer) {
    /* Reset timer */
    Timer->CNT = Timer->ARR;
...
TM_DELAY_Timer_t*
TM_DELAY_TimerAutoReload(TM_DELAY_Timer_t* Timer, uint8_t AutoReload) {
    /* Reset timer */
...
TM_DELAY_Timer_t*
TM_DELAY_TimerAutoReloadValue(TM_DELAY_Timer_t* Timer, uint32_t AutoReloadValue) {
    /* Reset timer */
