...
uint8_t TM_DS18B20_SetResolution(uint8_t *ROM, TM_DS18B20_Resolution_t resolution) {
	...
	if (resolution == TM_DS18B20_Resolution_9bit) {
		conf &= ~(1 << TM_DS18B20_RESOLUTION_R1);
		conf &= ~(1 << TM_DS18B20_RESOLUTION_R0);
	} else if (resolution == TM_DS18B20_Resolution_10bit) {
		conf &= ~(1 << TM_DS18B20_RESOLUTION_R1);
		conf |= 1 << TM_DS18B20_RESOLUTION_R0;
	} else if (resolution == TM_DS18B20_Resolution_11bit) {
		conf |= 1 << TM_DS18B20_RESOLUTION_R1;
		conf &= ~(1 << TM_DS18B20_RESOLUTION_R0);
	} else if (resolution == TM_DS18B20_Resolution_12bit) {
		conf |= 1 << TM_DS18B20_RESOLUTION_R1;
		conf |= 1 << TM_DS18B20_RESOLUTION_R0;
	}
