...
void
TM_GENERAL_DisableInterrupts(void) {
    /* Disable interrupts */
    __disable_irq();
...
uint8_t
TM_GENERAL_EnableInterrupts(void) {
...
uint32_t
TM_GENERAL_GetClockSpeed(TM_GENERAL_Clock_t clock) {
...
TM_GENERAL_GetResetSource(uint8_t reset_flags) {
...
uint8_t
TM_GENERAL_DWTCounterEnable(void) {
    uint32_t c;
...
void
TM_GENERAL_ConvertFloat(TM_GENERAL_Float_t* Float_Struct, float Number, uint8_t decimals) {
    /* Check decimals */
    if (decimals > 9) {
        decimals = 9;
    }
...
float
TM_GENERAL_RoundFloat(float Number, uint8_t decimals) {
    float x;
...
uint32_t
TM_GENERAL_NextPowerOf2(uint32_t number) {
    /* Check number */
    if (number <= 1) {
        return 1;
    }
...
__weak void
TM_GENERAL_SystemResetCallback(void) {
