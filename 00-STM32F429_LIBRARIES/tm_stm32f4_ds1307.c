...
void
TM_DS1307_GetDateTime(TM_DS1307_Time_t* time) {
    uint8_t data[7];
...
void
TM_DS1307_SetDateTime(TM_DS1307_Time_t* time) {
    uint8_t data[7];
...
void
TM_DS1307_DisableOutputPin(void) {
    /* Set output pin to high */
    TM_DS1307_EnableOutputPin(TM_DS1307_OutputFrequency_High);
}

uint8_t
TM_DS1307_Bcd2Bin(uint8_t bcd) {
...
uint8_t
TM_DS1307_CheckMinMax(uint8_t val, uint8_t min, uint8_t max) {
    if (val < min) {
        return min;
    } else if (val > max) {
        return max;
