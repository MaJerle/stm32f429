...
uint8_t
TM_USB_VCP_BufferEmpty(void) {
    return (tm_int_usb_vcp_buf_num == 0);
}

uint8_t
TM_USB_VCP_BufferFull(void) {
    return (tm_int_usb_vcp_buf_num == USB_VCP_RECEIVE_BUFFER_LENGTH);
}

uint8_t
TM_USB_VCP_FindCharacter(volatile char c) {
    uint16_t num, out;
...
TM_USB_VCP_Result
TM_USB_VCP_Putc(volatile char c) {
...
TM_USB_VCP_Result
TM_USB_VCP_Send(uint8_t* DataArray, uint32_t Length) {
    /* Send array */
...
uint16_t
TM_USB_VCP_Gets(char* buffer, uint16_t bufsize) {
    uint16_t i = 0;
    uint8_t c;
...
TM_USB_VCP_Result
TM_INT_USB_VCP_AddReceived(uint8_t c) {
...
TM_USB_VCP_Result
TM_USB_VCP_GetSettings(TM_USB_VCP_Settings_t* Settings) {
    /* Fill data */
