...
void
TM_USART_DMA_Init(USART_TypeDef* USARTx) {
    /* Init DMA TX mode */
...
void
TM_USART_DMA_InitWithStreamAndChannel(USART_TypeDef* USARTx, DMA_Stream_TypeDef* DMA_Stream, uint32_t DMA_Channel) {
    /* Get USART settings */
...
DMA_Stream_TypeDef*
TM_USART_DMA_GetStream(USART_TypeDef* USARTx) {
    /* Get USART settings */
    return TM_USART_DMA_INT_GetSettings(USARTx)->DMA_Stream;
}

void
TM_USART_DMA_Deinit(USART_TypeDef* USARTx) {
    /* Get USART settings */
    TM_USART_DMA_INT_t* Settings = TM_USART_DMA_INT_GetSettings(USARTx);
...
uint8_t
TM_USART_DMA_Puts(USART_TypeDef* USARTx, char* DataArray) {
    /* Call DMA Send function */
    return TM_USART_DMA_Send(USARTx, (uint8_t*)DataArray, strlen(DataArray));
}

uint16_t
TM_USART_DMA_Sending(USART_TypeDef* USARTx) {
    /* Get USART settings */
    TM_USART_DMA_INT_t* Settings = TM_USART_DMA_INT_GetSettings(USARTx);
...
void
TM_USART_DMA_EnableInterrupts(USART_TypeDef* USARTx) {
    /* Get USART settings */
    TM_USART_DMA_INT_t* Settings = TM_USART_DMA_INT_GetSettings(USARTx);
...
void
TM_USART_DMA_DisableInterrupts(USART_TypeDef* USARTx) {
    /* Get USART settings */
    TM_USART_DMA_INT_t* Settings = TM_USART_DMA_INT_GetSettings(USARTx);
...
/* Private functions */
static TM_USART_DMA_INT_t*
TM_USART_DMA_INT_GetSettings(USART_TypeDef* USARTx) {
