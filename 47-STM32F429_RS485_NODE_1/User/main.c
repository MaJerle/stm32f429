/**
 *	Keil project example for RS-485 communication, node 1
 *
...
int main(void) {
	...
	/* Initialize RS-485, node 1 */
	/* Our ID = 0x01, use 115200 baud for USART communication */
	TM_RS485_Init(&RS485, 0x01, 115200);
	...
	while (1) {
		...
		if (TM_DISCO_ButtonPressed() && !buttonPressed) {
			...
			/* Send data to RS-485 */
			/* Send 5 bytes to RS-485, send to node with ID = 0x02 */
			TM_RS485_Send(&RS485, 0x02, RS485_Data, 5);
